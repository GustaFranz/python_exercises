# DEMANDA
# Empresa: Papelaria Central
# Setor: Varejo escolar
# Solicitacao: Simular campanha com 10% de desconto em todos os itens.

# EXERCICIO 07 - Dict comprehension: desconto em precos (contexto corporativo)
#
# precos = {"caderno": 12.0, "caneta": 3.5, "borracha": 2.0, "estojo": 25.0}
# DESCONTO = 10  # percentual
# Gere precos_promocao com dict comprehension aplicando o desconto.
# Exiba tabela comparativa: produto | preco original | preco promocional.
#
# ORIENTACOES
## Formula: preco - (preco * DESCONTO / 100)
## Sintaxe: {produto: novo_preco for produto, preco in precos.items()}
## Arredonde com round(valor, 2) se necessario.

# --- Implemente sua solucao abaixo ---
#
# =============================================================================
# # RESOLUCAO DO EXERCICIO
# =============================================================================

precos = {"caderno": 12.0, "caneta": 3.5, "borracha": 2.0, "estojo": 25.0}

DESCONTO = 10
precos_promocao = {chave: valor - (valor * DESCONTO/100) for chave, valor in precos.items()}
print("\n======================================================")
print("------------ TABELA COMPARATIVA DE PREÇOS ------------")
print("======================================================")
print()
print(f'{"PRODUTO":<10} | {"PREÇO ORIGINAL":>14} | {"PREÇO COM DESCONTO":>18} {"|":>5}')
print("-" * 54)
for produto, preco_original in precos.items():
    preco_desconto = precos_promocao[produto]
    print(f'{produto:<10} | {preco_original:>14} | {preco_desconto:>18} {"|":>5}')
print()

# =============================================================================
# # APRENDIZADOS E CONSOLIDACAO DE CONCEITOS
# =============================================================================

#
#
# OBRIGADO!
# FIQUE A VONTADE PARA CONTRIBUIR COM O MEU APRENDIZADO
